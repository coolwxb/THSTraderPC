...
def record_start():
    ...
    if os.path.exists(path):
        ...
        df = pd.DataFrame(
            columns=['代码','名称', '紫色线', '灰色线1', '灰色线2', '收盘价', '收盘-紫色', '收盘-灰色1', '收盘-灰色2',
...
def record_end():
    ...
    # 将dic 更新输出到excel
    df = pd.DataFrame(
        columns=['代码','名称', '紫色线', '灰色线1', '灰色线2','最低价','最高价','收盘价','涨跌幅', '收盘-紫色', '收盘-灰色1', '收盘-灰色2', '紫色触发', '灰色1触发', '灰色2触发', '紫色买入盈亏', '灰色1买入盈亏', '灰色2买入盈亏', '买入日期', '卖出日期'])
    for key in dic:
        j = dic[key]
        try:
            df = df.append(
