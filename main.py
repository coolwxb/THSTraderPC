...
def watch_file(path = '预警.txt', share_lock = shared_lock):
    ...
    while True:
        if is_time_to_sell(10, 45):
            ...
            msg.dingding.send_msg(f"当前时间是上午10点55分，取消所有挂单\r\n")
            thsObj.quxiao()
                        ...
                        if not flag:
                            ...
                            with share_lock:
                                ...
                                if purple_price == 0 and gray_price_up == 0 and gray_price_down == 0:
                                    ...
                                    msg.dingding.send_msg(f"{parsed_code}  {stock_name} 的灰色线在紫色线上方，不符合强势条件")
                                else:
                                    ...
                                    if current <= gray_price_down and gray_price_down != 0:
                                        ...
                                        msg.dingding.send_msg(f"当前{parsed_code}  {stock_name} 买入价格为{current}")
                                        ...
                                        msg.dingding.send_msg(f"当前{parsed_code}  {stock_name}买入价格为{gray_price_up}")
                                        ...
                                        msg.dingding.send_msg(f"当前{parsed_code}  {stock_name}买入价格为{purple_price}")
